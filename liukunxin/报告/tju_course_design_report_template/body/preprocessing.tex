...
图\ref{fig::preprocessing1}展示了一张RLE解码后的图像，高亮部分表示被标记的船舶。
...
\begin{figure}[htbp]
    \centering
    \subfigure{
        \includegraphics[width=0.3\linewidth]{body/preprocessing_pic/2_1}
    }
    \subfigure{
        \includegraphics[width=0.3\linewidth]{body/preprocessing_pic/2_2}
    }
    \subfigure{
        \includegraphics[width=0.3\linewidth]{body/preprocessing_pic/2_3}
    }
    \caption{非法的图像}
...
图\ref{fig::preprocessing3}展示了处理后文件大小的分布图，可以看出文件大小较为均匀地分布在120Kb左右。
...
结果如图\ref{fig::preprocessing6}，其中图一表示了卫星图像原图像，图二表示了在卫星图像的船只标注结果，图三表示了卫星图像原图像与标注结果相结合的图像。
